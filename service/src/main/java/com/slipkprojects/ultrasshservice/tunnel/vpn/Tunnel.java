...
public class Tunnel
{
							 ...
							 boolean enabledFilter, boolean filterBypassMode, String[] filterApps, boolean enableTethering) throws java.lang.Exception
	{
		...
		try
		{
			...
			if (enableTethering) {
                ...
                mRoutes.addIP(new CIDRIP("192.168.43.0", 24), false);
			...
			if (Build.VERSION.SDK_INT >= 21) {
				...
				if (enabledFilter) {
					...
					builder.addDisallowedApplication("com.magicvpn.net"); // slow
					mHostService.onDiagnosticMessage(String.format("SlowDNS Ativado ✔️ ")); // slow
	...
	private boolean routeThroughTunnel(final String socksServerAddress, final String[] dnsResolver, boolean forwardDns, final String udpResolver, final boolean transparentDns)
	{
		...
		mTun2Socks.setOnTun2SocksListener(new Tun2Socks.OnTun2SocksListener(){
				...
				public void onStart()
				{
					mHostService.onDiagnosticMessage("<font color=green><strong>CONECTADO</strong>");
				}
				@Override
				public void onStop()
				{
					mHostService.onDiagnosticMessage("<font color=red><strong>DESCONECTADO</strong>");
					stop();
				}
			});

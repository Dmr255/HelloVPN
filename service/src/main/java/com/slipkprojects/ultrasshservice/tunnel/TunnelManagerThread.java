...
ServerHostKeyVerifier, DebugLogger
{
	...
	private static boolean fakeTunnel;

	// WakeLock Dados em segundo plano >>>
	
	private PowerManager getSystemService(String POWER_SERVICE){return null;}
	...
	private void setWakelock()
	{
        try
		{
			...
			this.wakeLock = getSystemService(PowerManager).newWakeLock(1, " com.slipkprojects.ultrasshservice.TunnelManagerThread");
            this.wakeLock.acquire();
        }
		catch (Exception e)
		{
			SkStatus.logInfo("Wakelock 📶 ✔️");
	...
	public void run()
	{
		...
		SkStatus.logInfo("<font color=#ffa500><strong>Iniciando Tunnel SSH</strong>");
		...
		while (!mStopping) {
			try {
				if (!TunnelUtils.isNetworkOnline(mContext)) {
					...
					if (tries > 100)
						SkStatus.logInfo("<font color=#ffa500><strong>RECONECTANDO</strong>");
				...
				SkStatus.logError("<font color=red><strong>DESCONECTADO</strong>");
				closeSSH();
	...
	public void stopAll() {
		...
		SkStatus.logInfo("<font color=#ffa500><strong>Fechando Tunnel SSH</strong>");
		...
		new Thread(new Runnable() {
				...
				public void run() {
					...
					mHandler.post(new Runnable() {
							...
							@Override
							public void run() {
								TastyToast.makeText(mContext, "DESCONECTADO",TastyToast.LENGTH_SHORT,TastyToast.ERROR);
							}
						});
				...
				public void run() {
					while (true) {
						...
						if (lastPingLatency > 0) {
							SkStatus.logInfo(String.format("Latência: %d ms", lastPingLatency));
							break;}}}
...
//					
//					if (lastPingLatency > 0) {
//						SkStatus.logInfo(String.format("Ping: %d ms", lastPingLatency));
//						break;
//					}
//				}
//			}
...
//	}
	...
	private final static int AUTH_TRIES = 100;
	private final static int RECONNECT_TRIES = 100;
		...
		try {
			...
			mHandler.post(new Runnable() {
					...
					public void run() {
						...
						TastyToast.makeText(mContext, "CONECTADO",
	...
	protected void conectar(String servidor, int porta) throws Exception {
		...
		try {
			...
			if (useProxy && cause.contains("Key exchange was not finished")) {
				...
				SkStatus.logError("Erro! <br>Sinal indisponivel <br>tente mais tarde.");

				mHandler.post(new Runnable() {
						@Override
						public void run() {
							TastyToast.makeText(mContext, "ERRO AO CONECTAR",TastyToast.LENGTH_SHORT,TastyToast.CONFUSING);
						}
					});
			}
	...
	protected void autenticar(String usuario, String senha, String keyPath) throws IOException {
		...
		try {
			if (mConnection.isAuthMethodAvailable(usuario,
				...
				SkStatus.logInfo("Verificando acesso...");

				if (mConnection.authenticateWithPassword(usuario,
														 senha)) {
					SkStatus.logInfo("<font color=green><strong>Autenticado com sucesso</strong>");
				}
			}
		...
		if (!mConnection.isAuthenticationComplete()) {
			SkStatus.logInfo("<font color=#ffa500><strong>Falha ao autenticar<br>verifique seu acesso e tente novamente</strong>");
		...
		if (mTunnelType != 0) {
			...
			switch (mTunnelType) {
				case Settings.bTUNNEL_TYPE_SSH_DIRECT:
					SkStatus.logInfo("Conexão:SSH+DIRECT");
...
//								SkStatus.logInfo("Payload: " + mCustomPayload);
				...
				case Settings.bTUNNEL_TYPE_SSH_PROXY:
					SkStatus.logInfo("Conexão:SSH+PROXY");
				...
				case Settings.bTUNNEL_TYPE_SSH_SSL:
					SkStatus.logInfo("Conexão:SSH+SSL");
				...
				case Settings.bTUNNEL_TYPE_PAY_SSL:
					SkStatus.logInfo("Conexão:PAY+SSL");
				...
				case Settings.bTUNNEL_TYPE_SLOWDNS:
					SkStatus.logInfo("Conexão:SLOWDNS");
                    if (mCustomPayload != null) {
                        try {

...
import android.content.SharedPreferences; //Slowdns
import com.slipkprojects.ultrasshservice.tunnel.DNSTunnelThread; //Slowdns
...
implements SkStatus.StateListener
{
	...
	private DNSTunnelThread mDnsThread; //Slowdns
	...
	public void onCreate()
	{
		...
		wakeLock = ((PowerManager)getSystemService(Context.POWER_SERVICE)).newWakeLock(1, "MAGIC VPN::WakelockService"); // Wakelock
	...
	protected void networkStateChange(Context context, boolean showStatusRepetido) {
		...
		try {
			...
			if (networkInfo == null) {
				...
				netstatestring = String.format("REDE %1$s %4$s %3$s", networkInfo.getTypeName(),
